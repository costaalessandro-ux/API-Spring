...
public class MedicoController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity cadastrar(@RequestBody @Valid DadosCadastradosMedico dados, UriComponentsBuilder uriBuilder){
    ...
    public ResponseEntity<Page<DadosListagemMedico>> Listar(@PageableDefault(size = 10, sort = {"nome"}) Pageable paginacao) {
        var page = repository.findAllByAtivosTrue(paginacao).map(DadosListagemMedico::new);
